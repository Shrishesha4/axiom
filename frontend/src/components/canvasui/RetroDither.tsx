...
export interface RetroDitherElements {
  ...
  /** Element that receives pointer events for the lens. Defaults to the output parent. */
...
export function createRetroDither(
  ...
  const { source, content, output, listenTarget: listenRoot } = elements;
  ...
  async function captureViaDomSnapshot() {
    if (htmlInCanvas || destroyed || captureInflight) return;
    const width = Math.max(1, content.clientWidth);
    const height = Math.max(1, content.clientHeight);
  ...
  function scheduleContentCapture() {
    ...
    captureTimer = window.setTimeout(() => {
      captureTimer = 0;
      captureStamp = performance.now();
      void captureViaDomSnapshot();
    }, wait);
  }
    ...
    instanceRef.current = createRetroDither(
      {
        ...
        content,
        output,
        listenTarget: listenTargetRef?.current ?? undefined,
      },
      initialOptions,
    );
